...
class ChessboardHistory:
    ...
    def _gen_evenly(self, len: int) -> List[str]:
        ...
        """
        l = np.int16(np.linspace(1, 255, len)).tolist()
    ...
    def _gen_exp(self, len: int, base: int = 10) -> List[str]:
        """Creates a list of opacities that range from 0-255 in hex format.
        The value increases exponentially until 255 (in hex).

        Args:
            len: length of input to match
            base: ]1;inf] determines the growth rate of the exp function.
        """
        l=list(np.int16(np.power(base,np.linspace(-1,0, len))*255))
    ...
    def print_history(self, pgn: str, rgb: str, fade_function: str = "evenly") -> None:
        ...
        """
        if fade_function not in ["evenly", "last2","exp"]:
        ...
        elif fade_function == "exp": 
            opacities = self._gen_exp(move_count)
        else:
...
if __name__ == "__main__":
    ...
    ch.print_history(pgn, "#ff0000", fade_function="exp")
